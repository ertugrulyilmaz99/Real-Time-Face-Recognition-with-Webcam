...
class CasperFaceID:
    ...
    def addPerson(self,cameraId):
        from PIL import Image
        popup = tk.Tk()

        personName = simpledialog.askstring("Input String","Enter the Persons Name")
        popup.destroy()
        popup.mainloop()
        ...
        while True:
            ...
            while countForSavedImages <= 5:
                ...
                for face_location in face_locations:
                    ...
                    countForSavedImages = countForSavedImages + 1
    ...
    def detect(self,cameraId):
        ...
        known_face_names = [0 for x in range(number_files)]
        image_of_people = [0 for x in range(number_files)]
        known_face_encodings = [0 for x in range(number_files)]
        ...
        for i in range(len(list)):
            known_face_names[i] = list[i].split('-')[0]

            image_of_people[i] = face_recognition.load_image_file(files[i])

        for i in range(len(list)):
            known_face_encodings[i] = face_recognition.face_encodings(image_of_people[i])[0]
        ...
        while True:
            ...
            for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
                ...
                if matches[best_match_index]:
                    ...
                    if self.canOpen and face_distances[best_match_index] <0.50:
